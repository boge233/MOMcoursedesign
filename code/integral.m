function [output] = integral(triangletable,pointtable,edge1,edge2,trib,trit)
...
if length(overlap) == 0
    ...
    for i = 1:4
        for z = 1:4
            ...
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(point1(i,:)-point3(z,:)))/abs(point1(i,:)-point3(z,:));
        end
    end
    for i = 1:4
        for z = 1:4
            ...
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(point1(i,:)-point4(z,:)))/abs(point1(i,:)-point4(z,:));
        end
    end
    for i = 1:4
        for z = 1:4
            ...
            [rou2,s2] = rwg(point3(z,:),edge2,triangletable(trit(1),:),triangletable(trit(2),:),pointtable);
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(point2(i,:)-point3(z,:)))/abs(point2(i,:)-point3(z,:));
        end
    end
    for i = 1:4
        for z = 1:4
            [rou1,s1] = rwg(point2(i,:),edge1,triangletable(trib(1),:),triangletable(trib(2),:),pointtable);
            [rou2,s2] = rwg(point4(z,:),edge2,triangletable(trit(1),:),triangletable(trit(2),:),pointtable);
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(point2(i,:)-point4(z,:)))/abs(point2(i,:)-point4(z,:));
        end
    end
    edgep1 = pointtable(edge1(1)) - pointtable(edge1(2));
    edgep2 = pointtable(edge2(1)) - pointtable(edge2(2));
    I = Ippmm * abs(edgep1)*abs(edgep2)/(4*pi);
elseif length(overlap) == 1
    ...
    %奇异性处理
    [I1,I2] = analyI12(v1,v2,v3,vn,vm,sign);
    pointb = zeros(4,3,2);
    pointt = zeros(4,3,2);
            ...
            for i = 1:4
                for z = 1:4
                    ...
                    if page1 == index1&&page2 == index2
                        edgep1 = pointtable(edge1(1)) - pointtable(edge1(2));
                        edgep2 = pointtable(edge2(1)) - pointtable(edge2(2));
                        Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*j*k*(-1)*abs(edgep1)*abs(edgep2)/(4*pi);
                        Ippmm = Ippmm + I1+I2;
                    else
                        Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(pointb(i,:,page1)-pointt(z,:,page2)))/abs(pointb(i,:,page1)-point3(z,:,page2))*abs(edgep1)*abs(edgep2)/(4*pi);
                    end
                end
            end
        end
    ...
    for i = 1:4
        for z = 1:4
            [rou1,s1] = rwg(point1(i,:),edge1,triangletable(trib(1),:),triangletable(trib(2),:),pointtable);
            [rou2,s2] = rwg(point3(z,:),edge2,triangletable(trit(1),:),triangletable(trit(2),:),pointtable);
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*(-j*k)*abs(edgep1)*abs(edgep2)/(4*pi);
    ...
    for i = 1:4
        for z = 1:4
            [rou1,s1] = rwg(point1(i,:),edge1,triangletable(trib(1),:),triangletable(trib(2),:),pointtable);
            [rou2,s2] = rwg(point4(z,:),edge2,triangletable(trit(1),:),triangletable(trit(2),:),pointtable);
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(point1(i,:)-point4(z,:)))/abs(point1(i,:)-point4(z,:))*abs(edgep1)*abs(edgep2)/(4*pi);
        end
    end
    for i = 1:4
        for z = 1:4
            [rou1,s1] = rwg(point2(i,:),edge1,triangletable(trib(1),:),triangletable(trib(2),:),pointtable);
            [rou2,s2] = rwg(point3(z,:),edge2,triangletable(trit(1),:),triangletable(trit(2),:),pointtable);
            Ippmm = Ippmm + weight(i)*weight(z)* (j*w*mu/4 * rou1*rou2'-s1*s2*j/(w*epsilon))*exp(-j*k*abs(point2(i,:)-point3(z,:)))/abs(point2(i,:)-point3(z,:))*abs(edgep1)*abs(edgep2)/(4*pi);
...
function [I1,I2] = analyI12(v1,v2,v3,vn,vm,sign)
%输入为点本身 sign是I1的符号
syms a b c d
 %% 初始计算公式表达式
